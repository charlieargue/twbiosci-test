...
(function (window) {
	...
	function Controller(model, view) {
		...
		// CLEAR COMPLETED
		$live('#todoapp #clear-completed', 'click', function () {
			that.clearCompleted();
		});
	...
	Controller.prototype.showActive = function () {
		var that = this;
		that.model.read(function (data) {
			data = data.filter((d) => !!d.complete === false);
			that.view.render('showEntries', data);
		});
	};
	...
	Controller.prototype.addItem = function (title) {
		...
		that.model.create(title, false, function () {
	...
	/**
	 NOTE: should really be called toggle item completeness
	 */
	Controller.prototype.completeItem = function (item) {
		var that = this;
		that.model.read(item.id, function (data) {
			that.model.update(data[0].id, { complete: !!data[0].complete === true ? false : true }, function () {
				if (that._activeRoute !== 'All' && data.complete === true) {
					that.view.render('completeItem', data.id);
				}
			});
		});
	...
	Controller.prototype.clearCompleted = function (id) {
		var that = this;
		that.model.read(function (data) {
			that.model.removeCompleted(function () {
				data.forEach((d) => {
					if (d.complete === true) {
						that.view.render('removeItem', d.id);
					}
				});
			});
		});
